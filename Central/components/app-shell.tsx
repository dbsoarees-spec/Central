...
const navigation: Array<{
  ...
  { href: "/vistorias", label: "Vistoria de veículos", icon: Icons.truck, roles: ["ADMIN", "GERENCIA"] },
...
function canAccessPath(pathname: string, role: Role) {
  if (role === "VENDEDOR" && /^\/vendas\/[^/]+\/editar\/?$/.test(pathname)) return false;
  const item = navigation.find((entry) => pathname === entry.href || pathname.startsWith(`${entry.href}/`));
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const visibleNavigation = user ? navigation.filter((item) => item.roles.includes(user.role)) : navigation;
  const currentLabel = navigation.find((item) => pathname === item.href || pathname.startsWith(`${item.href}/`))?.label ?? "Central Express";
  ...
  return (
    <div className={`app-frame ${collapsed ? "sidebar-collapsed" : ""}`}>
      <aside className={`sidebar ${mobileOpen ? "mobile-open" : ""}`}>
        <div className="brand-row"><Link href="/inicio" className="brand" aria-label="Central Express — Início"><span className="brand-logo" aria-hidden="true" /><span className="brand-copy"><strong>Central Express</strong><small>Frete</small></span></Link><button className="sidebar-close" onClick={() => setMobileOpen(false)} aria-label="Fechar menu"><Icons.close /></button></div>
        <nav className="main-nav" aria-label="Navegação principal">
          {visibleNavigation.map((item) => { const active = pathname === item.href || pathname.startsWith(`${item.href}/`); const Icon = item.icon; return <Link key={item.href} href={item.href} onClick={() => setMobileOpen(false)} className={active ? "active" : ""} aria-current={active ? "page" : undefined}><Icon /><span>{item.label}</span></Link>; })}
        </nav>
      ...
      <div className="app-main">
        ...
        <main className="page-content">{userLoaded && !allowed ? <section className="panel"><span className="eyebrow">Acesso restrito</span><h2>Seu perfil não permite abrir esta tela.</h2><p>Vendedores podem consultar suas próprias vendas e comissões, mas não podem editar uma venda depois de salva.</p></section> : children}</main>
      </div>
    </div>
  );
}
